...
namespace models.Organization
{
	class Organisation
	{
		...
		{
			DataBaseAzat DB = new DataBaseAzat(request: $"SELECT * FROM organisation WHERE org_id={id}");
		...
		{
			DataBaseAzat DB = new DataBaseAzat(request: $"SELECT * FROM organisation WHERE distr_id={id}");
